...
Note that our confidence interval does not contain 0, and our p-value is well under .05, `r toString(pvalue)`, so we reject the null hypothesis. But our R-squared is very small `r toString(rsquared)`. Essentially, wind does have predictive power for passing game success, but that effect has a high variation.
