...
Conventional wisdom is that NFL teams don't like to pass and don't have success passing in high wind, but the data suggests there is a lot of variance in these numbers.

### Teams seem to pass less in division games and also have less success passing.

```{r}
ggplot(data, aes(x= factor(div_game), y=pass_rate)) + geom_boxplot(fill = "lightblue", color = "darkblue", alpha = 0.7) + labs(title = "Pass Rate Division vs Other", x = "Division Game", y = "Pass_ Rate")

ggplot(data, aes(x= factor(div_game), y=pass_epa_game)) + geom_boxplot(fill = "lightblue", color = "darkblue", alpha = 0.7) + labs(title = "Pass EPA Division vs Other", x = "Division Game", y = "Pass_EPA")
