...
```
The most obvious difference in these models is the number of predictors. The Vegas model (top) underwent step wise backward selection and only depends on 6 predictors, whereas the ridge model (middle) and lasso model (bottom) depend on 16 and 15 non-zero predictors respectively. 
...
ridco[8]
```
This is the ridge regression's version of each of the vegas model coefficients
...
lasco[8]
```
This is the lasso regression's version of each of the vegas model coefficients
...
ridco[7]
ridco[8]
print('above is ridge, below is lasso')
lasco[1]
lasco[9]
lasco[10]
lasco[4]
